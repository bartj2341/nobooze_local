    ...
    <div class="article-banner__bg p-absolute w-100 h-100">
        ...
        <?php if($image): ?>
            <div class="jarallax jarallax-up article-banner__image w-100 h-100">
                <img class="jarallax-img article-banner__img <?= $image_position ?>" src="<?= $image["url"] ?>" alt="<?= $image["alt"] ?>" />
            </div>
        <?php elseif(get_the_post_thumbnail()): ?>
            <div class="jarallax jarallax-up article-banner__image w-100 h-100">
                <img class="jarallax-img article-banner__img <?= $image_position ?>" src="<?php the_post_thumbnail_url(); ?>" alt="<?= get_post_meta( get_post_thumbnail_id(), '_wp_attachment_image_alt', true ); ?>" />
            </div>
        <?php endif; ?>   
    </div>
